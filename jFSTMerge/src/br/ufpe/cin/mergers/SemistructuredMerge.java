...
public final class SemistructuredMerge {
	...
	public static String merge(File left, File base, File right,
		...
		return FilesManager.indentCode(Prettyprinter
				.print(context.superImposedTree));
	}
	...
	private static void mergeMatchedContent(FSTNode node, MergeContext context)
		...
		if ((node instanceof FSTNonTerminal)) {
			...
			if (((FSTTerminal) node).getBody().contains("##FSTMerge##")) {
				...
				String mergedBodyContent = TextualMerge.merge(leftContent,
						baseContent, rightContent, true);
				((FSTTerminal) node).setBody(mergedBodyContent);

				identifyNodesEditedInOnlyOneVersion(node, context, leftContent,
						baseContent, rightContent);

				identifyPossibleNodesDeletionOrRenamings(node, context,
						leftContent, baseContent, rightContent);

				findConsecutiveLinesOrSpacingConflicts(node, context,
						leftContent, baseContent, rightContent,
						mergedBodyContent);
			}
		} else {
	...
	private static void findConsecutiveLinesOrSpacingConflicts(FSTNode node,
		...
		if ((node.getType().equals("MethodDecl"))
			...
			if (mergedBodyContent.contains("<<<<<<")) {
				if (!FPFNUtils.isRenamingOrDeletionConflict(leftContent,
						baseContent, rightContent)) {
					String mergedBodyContentInclBase = new TextualMerge()
					.mergeIncludingBase(leftContent, baseContent,
							rightContent, node);
					List<MergeConflict> mergeConflictsOriginal = FilesManager
							.extractMergeConflicts(mergedBodyContent);
					List<MergeConflict> mergeConflictsInclBase = FilesManager
							.extractMergeConflicts(mergedBodyContentInclBase);
					List<MergeConflict> mergeConflicts = FilesManager
							.filterNonOverlapingMergeConflicts(
									mergeConflictsInclBase,
									mergeConflictsOriginal);
					for (MergeConflict mc : mergeConflicts) {
						...
						diff.signature = FPFNUtils
								.extractSignature(mergedBodyContentInclBase);
	...
	private static void identifyPossibleNodesDeletionOrRenamings(FSTNode node,
			MergeContext context, String leftContent, String baseContent,
			String rightContent) {
		String leftContenttrim = FilesManager
				.getStringContentIntoSingleLineNoSpacing(leftContent);
		String baseContenttrim = FilesManager
				.getStringContentIntoSingleLineNoSpacing(baseContent);
		String rightContenttrim = FilesManager
				.getStringContentIntoSingleLineNoSpacing(rightContent);
