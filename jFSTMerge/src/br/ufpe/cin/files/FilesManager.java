...
public final class FilesManager {
	...
	public static boolean areFilesDifferent(File left, File base, File right,String outputFilePath, MergeContext context) {
		...
		//FPFN merging only complete tuples
	...
	public static void main(String[] args) {
		String example = "public void m(){\n" +
				...
				">>>>>>> /Users/paolaaccioly/Desktop/Teste/jdimeTests/right/Example.java\n" +
				"    }";
		List<MergeConflict> mcs = FilesManager.extractMergeConflicts(example);
		System.out.println(mcs);
	}

	//FPFN
	public static List<MergeConflict> filterNonOverlapingMergeConflicts(
			List<MergeConflict> mergeConflictsToBeFiltered,
			List<MergeConflict> baseComparison) {
		List<MergeConflict> result = new ArrayList<MergeConflict>();
		for(MergeConflict mco : baseComparison){
			for(MergeConflict mcf : mergeConflictsToBeFiltered){
				String lmco = FilesManager.getStringContentIntoSingleLineNoSpacing(mco.left);
				String rmco = FilesManager.getStringContentIntoSingleLineNoSpacing(mco.right);

				String lmcf = FilesManager.getStringContentIntoSingleLineNoSpacing(mcf.left);
				String rmcf = FilesManager.getStringContentIntoSingleLineNoSpacing(mcf.right);
